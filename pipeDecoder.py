...
try:
	command = askString("Pipe Decoder", "Shell command through which to pipe for decoding", "openssl enc -d -aes128 -nopad -K deadbeef -iv 0")
...
for r in ranges:
	...
	result = p.communicate(bytes)
	bytes_decoded = result[0]
	print "$ " + command
 	print result[1]
